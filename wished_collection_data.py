...
def get_wished_figure_by_uname(username, page_number):
    ...
    if response.status_code == 200:
        ...
        figure_list = []
        get_fig_list_url = f"https://myfigurecollection.net/?mode=view&username={username}&tab=collection&status=0&current=keywords&rootId=0&categoryId=-1&output=2&sort=category&order=asc&_tb=user&page={page_number}"
...
def get_wished_collection_max_page(username):
    url = f"https://myfigurecollection.net/?_tb=user&mode=view&username={username}&tab=collection&rootId=0&status=0"
